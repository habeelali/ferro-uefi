//! EFI_SYSTEM_TABLE. Console and runtime-services fields are still
//! null -- EFI_SIMPLE_TEXT_INPUT/OUTPUT_PROTOCOL and
//! EFI_RUNTIME_SERVICES don't exist yet, so there's nothing real to
//! point them at. BootServices is the real, working table from
//! boot_services.rs.
...
/// Must run after boot_services::init(). Wires in the real
/// BootServices pointer and FirmwareVendor, then computes a real
/// CRC32 (with crc32 zeroed during the calculation, per spec).
pub fn init() {
    unsafe {
